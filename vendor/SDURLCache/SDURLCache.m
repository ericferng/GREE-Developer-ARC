...
static float const kSDURLCacheDefault = 0; // Default cache expiration delay if none defined (0 sec)
...
@implementation NSCachedURLResponse(GreeNSCoder)
#pragma clang diagnostic push
#if 3 < __clang_major__ || ( 3 == __clang_major__ && 0 < __clang_minor__ )
// supported by Xcode 4.3 or later
...
@property (nonatomic, readwrite) NSUInteger diskCapacity;
- (void)periodicMaintenance;
@end

@implementation GreeSDURLCache

@synthesize diskCachePath, minCacheInterval, ioQueue, periodicMaintenanceOperation, ignoreMemoryOnlyStoragePolicy, diskPath;
...
+ (NSDate *)expirationDateFromHeaders:(NSDictionary *)headers withStatusCode:(NSInteger)status
{
    ...
    if (date)
    {
        now = [self.class dateFromHttpDateString:date];
    }
    else
    {
...
- (void)storeCachedResponse:(NSCachedURLResponse *)cachedResponse forRequest:(NSURLRequest *)request
{
    ...
    if ((storagePolicy == NSURLCacheStorageAllowed || (storagePolicy == NSURLCacheStorageAllowedInMemoryOnly && ignoreMemoryOnlyStoragePolicy))
        ...
        /*&& cachedResponse.data.length < self.diskCapacity*/) // comment outed to avoid access cachedResponse.data to avoid memory leaks
    {
